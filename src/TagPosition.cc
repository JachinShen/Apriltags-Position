...
void TagPosition::processImage(const cv::Mat& image, cv::Mat& image_gray)
{
    ...
    if(m_draw)
    {
        cv::Mat m_draw = image.clone();
        for(int i= 0; i < detections.size(); i++)
        {
            // also highlight in the image
            detections[i].draw(m_draw);
        }
        imshow(windowName, m_draw);  // OpenCV call
    }
...
float TagPosition::getBaseX()
{
    return base_position_x;
}

float TagPosition::getBaseY()
{
    return base_position_y;
}

void TagPosition::setVisability(bool visable)
{
    m_draw= visable;
}

bool TagPosition::getBaseDirection(float& baseDirectionCita)
{
